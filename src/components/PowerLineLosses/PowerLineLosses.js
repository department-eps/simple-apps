            ...
            <Container maxWidth={'lg'} className={styles['container']}>
                <Grid container spacing={2} className={styles['wrapper']}>
                    ...
                    <Grid item xs={12} md={12} lg={12} className={styles['item-ceneter']}>
                        <ModifiedPowerLineLossesScheme currentVoltageLosses={currentVoltageLosses} />
                    </Grid>
                    <Grid item xs={12} md={6} lg={3} className={styles['item-center']}>
                        <RadioButtons radioValue={radioValue} setRadioValue={setRadioValue} />
                    </Grid>
                    <Grid item xs={12} md={6} lg={3} className={styles['item-center']}>
                        <Sliders radioValue={radioValue} setVoltageLosses={setVoltageLosses} setCurrentVoltageLosses={setCurrentVoltageLosses} />
                    </Grid>
                    <Grid item xs={12} md={12} lg={12} sx={{ textAlign: 'center' }}>
                        <Typography className={styles['chart-label']}>
                            <span className={styles['katex-label']}>ΔU</span><span> [V]</span>
                        </Typography>
                        <Chart voltageLosses={voltageLosses} radioValue={radioValue} />
                    </Grid>
                </Grid>
            </Container>
